defmodule AshDecisions.Tck.ExpectedFailures do
  ...
  @entries %{
    {"compliance-level-3", "0076-feel-external-java"} =>
      "external Java functions: deliberately unsupported, and refused on safety grounds",
    {"compliance-level-3", "1111-feel-matches-function"} =>
      "engine validator requires an `id` the DMN schema makes optional (upstream)",
    {"compliance-level-3", "0087-chapter-11-example"} =>
      "engine validator rejects a decision carrying no logic (upstream)",
    ...
    {"compliance-level-2", "0008-LX-arithmetic"} => @arithmetic,
    {"compliance-level-2", "0009-invocation-arithmetic"} => @arithmetic,
    {"compliance-level-3", "0005-literal-invocation"} => @arithmetic,
    {"compliance-level-3", "0003-iteration"} => @arithmetic,
    {"compliance-level-3", "0004-lending"} => @arithmetic,
    {"compliance-level-3", "0014-loan-comparison"} => @arithmetic,
    {"compliance-level-3", "0040-singlenestedcontext"} => @arithmetic,
    {"compliance-level-3", "0041-multiple-nestedcontext"} => @arithmetic,
    {"compliance-level-3", "0100-arithmetic"} => @arithmetic
  }

  @doc "The reason this group is expected to fail, or nil if it is expected to pass."
  @spec reason(String.t(), String.t()) :: String.t() | nil
  def reason(level, group), do: Map.get(@entries, {level, group})
